...
void choose_number(t_sort **sort, t_sort **sort2, int count_one, int count_two, t_sort *one, t_sort *two, fdf *data)
{
	int op_next;
	int op_prev;
	char *str;
...
//	int stack[25] = {20, 40, 60, 80, 100, 120, 140, 150, 160, 180, 200, 220, 240, 260, 280, 300, 320, 340, 360, 380, 400, 420, 440, 460, 500};
	...
	while (i < 3) //для каждой группы выполняем необходимый код
	{
		...
		while (one != NULL)
		{
			...
			if (two == NULL)
			{
				choose_number(sort, sort2, count_one, INT32_MAX, one, two, data);
				break;
			}
			count_two = count_operations(two);
			choose_number(sort, sort2, count_one, count_two, one, two, data);
		}
		i++;
	}
...
//	int p = last_make_up(sort, sort2);
	int p = push_min_op(sort, sort2, data);
